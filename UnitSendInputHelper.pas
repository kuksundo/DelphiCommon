...
//2024.07.07: TList를 IList로 변경함 : 추후 Json으로 변환을 쉬베 하기 위해서임

unit UnitSendInputHelper;
...
type
  ...
  TSendInputHelper = class
  private
    FSendInputList: IList<TInput>;
  protected
  ...
  public
    ...
    class function GetMouseWheel(WheelData: Cardinal): TInputArray;
    ...
    procedure AddMouseWheel(WheelData: Cardinal);
    ...
    procedure AddInputArray(AAry: TInputArray);
    procedure InsertInputArray(AIndex: integer; AAry: TInputArray);

  published
    property SendInputList: IList<TInput> read FSendInputList;
  end;
...
procedure TSendInputHelper.AddMouseWheel(WheelData: Cardinal);
var
  Inputs: TInputArray;
begin
  Inputs := GetMouseWheel(WheelData);
...
begin
  ...
  FSendInputList.Add(DelayInput);
end;

procedure TSendInputHelper.AddInputArray(AAry: TInputArray);
var
  i: integer;
  LInput: TInput;
begin
...
begin
  FSendInputList.Add(GetKeyboardInput(VirtualKey, ScanCode, Flags, Time));
end;
...
constructor TSendInputHelper.Create;
begin
  FSendInputList := Collections.NewList<TInput>();
end;

destructor TSendInputHelper.Destroy;
begin
...
var
  ...
  begin
    SendInput(InputsCount, Inputs[0], SizeOf(TInput));
  end;

begin
  if FSendInputList.Count = 0 then
    Exit;

  // Neutralize the real current keyboard state
  if GetKeyState(VK_CAPITAL) = 1 then
  begin
    InsertInputArray(0, GetVirtualKey(VK_CAPITAL, True, True));
    AddVirtualKey(VK_CAPITAL, True, True);
  end;

  InputsCount := 0;
  SetLength(Inputs, FSendInputList.Count);

  for Input in FSendInputList do
  begin
...
var
  ...
  cc: Integer;
begin
  SetLength(Result, FSendInputList.Count);
  cc := 0;
...
begin
  ...
  Result.mi.time := Time;
  Result.mi.dwExtraInfo := 0;
end;

class function TSendInputHelper.GetMouseWheel(WheelData: Cardinal): TInputArray;
begin
  SetLength(Result, 1);
  Result[0] := GetMouseInput(0, 0, WheelData, MOUSEEVENTF_WHEEL, 0);
end;
...
procedure TSendInputHelper.InsertInputArray(AIndex: integer; AAry: TInputArray);
var
  i: integer;
  LInput: TInput;
begin
  for i := High(AAry) downto Low(AAry) do
  begin
    LInput := AAry[i];
    FSendInputList.Insert(i, LInput);
  end;
end;
