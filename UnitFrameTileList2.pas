...
uses
  ...
  GDIPPictureContainer, AdvGDIP, Data.DBXJSON, Data.DBXJSONCommon, JHP.BaseConfigCollect;

type
  TAddNewApp2List = procedure of object;

  TTileListFrame = class(TFrame)
...
function TTileListFrame.ConvertImage2String(APicture: TAdvGDIPPicture): string;
var
  LJA: TJSONArray;
  LMS: TMemoryStream;
begin
...
function TTileListFrame.ConvertString2Stream(AString: string): TMemoryStream;
var
  LJA: TJSONArray;
begin
  if AString = '' then
...
procedure TTileListFrame.DestroyVar;
begin
  DragAcceptFiles(Handle,False);
end;

procedure TTileListFrame.Initialize_list;
var
  tile: TAdvSmoothTile;
begin
  with tileList do
...
procedure TTileListFrame.InitVar;
begin
  DragAcceptFiles(Handle,True);
  Initialize_list;
end;

procedure TTileListFrame.SaveTile2File(ACollect: TpjhBase; AInitPath: string);
begin
  if AInitPath <> '' then
...
procedure TTileListFrame.tileListKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  //Delete
  if key = 46 then
...
procedure TTileListFrame.WMDropFiles(var msg: TMessage);
var
